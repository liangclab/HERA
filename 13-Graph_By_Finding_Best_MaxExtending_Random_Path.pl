...
my ($Contig_Head_Tail_Query, $Total_Pacbio_In_Contig) = &ReadingStartPoint;
my %Contig_Head_Tail_Query=%$Contig_Head_Tail_Query;
my %Total_Pacbio_In_Contig=%$Total_Pacbio_In_Contig;
...
#R498_3390190_1/90_20012/0_19922 1 19922 19922 Super-Scaffold_14.29 9130 29199 963453 0 99.13
	...
	$content[0]=~/(\S+)\/0_\d+$/;
	my $pacbio=$1;
	my $scaffold=$content[4];
	if($content[7]-$content[6]>=$content[5]){
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{0}=$content[8]; #Chain
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{1}=$content[5]; #Ref_start	
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{2}=$content[6]; #Ref_end
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{3}=$content[7]; #Ref_len
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{4}=$content[1]; #Qry_start
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{5}=$content[2]; #Qry_end
		$Scaffold_Contig_SEPoint{$scaffold}{'Head'}{$pacbio}{6}=$content[3]; #Qry_len
		$Total_Pacbio_In_Contig{$pacbio}{$total_count}{0}=$scaffold;
		$Total_Pacbio_In_Contig{$pacbio}{$total_count}{1}="Head";
		$total_count++;
	}
	else{
		$Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{0}=$content[8];
                $Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{1}=$content[5];
                $Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{2}=$content[6];
                $Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{3}=$content[7];
                $Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{4}=$content[1];
                $Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{5}=$content[2];
                $Scaffold_Contig_SEPoint{$scaffold}{'Tail'}{$pacbio}{6}=$content[3];
		$Total_Pacbio_In_Contig{$pacbio}{$total_count}{0}=$scaffold;
		$Total_Pacbio_In_Contig{$pacbio}{$total_count}{1}="Tail";           #Ctg head and tail
		$total_count++;
	}
    }
    close(Point);
    return(\%Scaffold_Contig_SEPoint,\%Total_Pacbio_In_Contig);
}
...
#		next if(exists $Reads_Pairs{$content[4]}{$content[0]}{$content[5]} && $Reads_Pairs{$content[4]}{$content[0]}{$content[5]}{'S'}>=$content[3]);
...
#		$ctg_ctg_chain{$QryStartHashArray[1]}{$QryStartHashArray[0]}{2}=$content[1];
		#May be replaced by ctg_ctg_ori
...
#			     my @neighbors=$Pair_Graph->neighbours($First_Pacbio);
			     my @neighbors=();
			     last if(!@neighbors);
...
############################### test ################################################
    ...
    foreach my $Scaffold (sort {$a cmp $b} keys %Scaffold_Contig_SEPoint){
          my $First_Scaffold_Contig=$Scaffold_Contig_SEPoint{$Scaffold}{'Tail'};
	  $sign++;
	  my $Second_Scaffold_Contig=$Scaffold_Contig_SEPoint{$Scaffold}{'Head'};
	  my %Bestn_PathWay=();
	  foreach my $First_Pacbio (keys %$First_Scaffold_Contig){

		my $Start_Ori="";
                if($Scaffold_Contig_SEPoint{$Scaffold}{'Tail'}{$First_Pacbio}{0}==0){
                        $Start_Ori="right";
                }
                elsif($Scaffold_Contig_SEPoint{$Scaffold}{'Tail'}{$First_Pacbio}{0}==1){
                        $Start_Ori="left";
                }
		...
		my %All_Path=();
                foreach my $Query (sort {$Reads_Pairs{$First_Pacbio}{$Start_Ori}{$b}{'S'} <=> $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$a}{'S'}} keys %$First_Temp){
                     ...
                     if(($Scaffold_Contig_SEPoint{$Scaffold}{'Tail'}{$First_Pacbio}{0}==0 && $Start_Ori eq "right") || (($Scaffold_Contig_SEPoint{$Scaffold}{'Tail'}{$First_Pacbio}{0}==1 && $Start_Ori eq "left"))){
			 my $current_path=$First_Pacbio."-".$Query;
			 my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Query}};
			 my $All_Path_Total=1;
			 for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
			     my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                             my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                             my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$Query}{0};
			     if((exists $Total_Pacbio_In_Contig{$Query}) && ($Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Query}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Query}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Query}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Query}{'O'} eq "right")) ){
					     $All_Path{$current_path}{0}=1;
					     if(!exists $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){
						 $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
				     	     }
				     	     else{
						 $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}++;
				             }
				             $All_Path{$current_path}{$All_Path_Total}=$Scaffold."-Tail-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-BestMatching-Passing";
					     $All_Path_Total++;
			     }
			 }
			 if(@Current_Pacbio_In_Contig==0){
		             $All_Path{$current_path}{0}=0;
			 }
			 $Current_Best++;
	             }
	      }
		     ...
		     foreach my $Current_Path (keys %All_Path){
			   next if($All_Path{$Current_Path}{0}==1);
			   ...
			   $Current_Best=0;
				     ...
				     if(exists $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}){
				       ...
				       my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$key1}};
                         	       my $All_Path_Total=1;
				       for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
					 my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                                         my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                                         my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$key1}{0};
					 if( (exists $Total_Pacbio_In_Contig{$key1}) && ($Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "right")) ){
						if(!exists $All_Path{$Current_Path}){
							    $All_Path{$extended_path}{0}=1;
                        	                            if(!exists $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){
                                	                          $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
                                        	            }
                                               		    else{
					                    ...
					                    $Current_Best++;
						            $All_Path{$extended_path}{$All_Path_Total}=$Scaffold."-Tail-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-BestMatching-Passing";
							    $All_Path_Total++;
						 }
						 else{
							    $All_Path{$extended_path}{0}=1;
	      ...
	      foreach my $best (keys %All_Path){
                         if($All_Path{$best}{0}==1){
                             my @content=split /-/,$best;
			     my $found_path=$All_Path{$best};
			     foreach my $many_path (keys %$found_path){
				next if ($many_path==0);
                             	print OUT "@content\t$All_Path{$best}{$many_path}\n";
			     }
                             for(my $j=0;$j<@content-1;$j++){
				 my $First_Chain=0;
				 my $Second_Chain=0;
	  ...
	  foreach my $Second_Pacbio (keys %$Second_Scaffold_Contig){
		...
		my $Start_Ori="";
                if($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==0){
                        $Start_Ori="left";
                }
                elsif($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==1){
                        $Start_Ori="right";
                }
                my $Second_Temp=$Reads_Pairs{$Second_Pacbio}{$Start_Ori};


                my $Current_Best=0;
                my %All_Path=();
                foreach my $Query (sort {$Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$b}{'S'} <=> $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$a}{'S'}} keys %$Second_Temp){
                     last if($Current_Best>=$Bestn);
                     next if($Second_Pacbio eq $Query);
                     if(($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==0 && $Start_Ori eq "left") || (($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==1 && $Start_Ori eq "right"))){
                          my $current_path=$Query."-".$Second_Pacbio;
			  my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Query}};
                          my $All_Path_Total=1;
			  for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
		             my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                             my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                             my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$Query}{0};
                             if((exists $Total_Pacbio_In_Contig{$Query}) && ($Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "right"))){
                                     $All_Path{$current_path}{0}=1;
                                     if(!exists $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){

                                         $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
                                     }
                                     else{
                                         $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}++;
                                     }
                                     $All_Path{$current_path}{$All_Path_Total}=$Scaffold."-Head-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-BestMatching-Passing";
				     $All_Path_Total++;
                             }
			   }
                           if(@Current_Pacbio_In_Contig==0){
              ...
              while($If_Extend==1){
                     ...
                     foreach my $Current_Path (keys %All_Path){
                           next if($All_Path{$Current_Path}{0}==1);
                           my @extend_line=split /-/,$Current_Path;
                           my $right_extend=$extend_line[0];

			   my $Current_Start_Ori="";
                           if(exists $Reads_Pairs{$right_extend}{'left'}{$extend_line[1]}){
                                    $Current_Start_Ori="right";
                           }
                           elsif(exists $Reads_Pairs{$right_extend}{'right'}{$extend_line[1]}){
                                    $Current_Start_Ori="left";
                           }
                           ...
                           foreach my $key1 (sort {$Reads_Pairs{$right_extend}{$Current_Start_Ori}{$b}{'S'} <=> $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$a}{'S'}}keys %$temp ){
                                     last if($Current_Best>=$Bestn);
                                     next if($key1 eq $extend_line[1]);
                                     next if($Current_Path=~/$key1/);
                                     if(exists $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}){
                                       my $extended_path=$key1."-".$Current_Path;
			 	       my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$key1}};
                                       my $All_Path_Total=1;
				       for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
					 my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                                         ...
                                         if((exists $Total_Pacbio_In_Contig{$key1}) && ($Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "right")) ){
                                                if(!exists $All_Path{$Current_Path}){
                                                    ...
                                                    $Current_Best++;
                                                    $All_Path{$extended_path}{$All_Path_Total}=$Scaffold."-Head-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-BestMatching-Passing";
						    $All_Path_Total++;
                                                 }
                                                 else{
                                                    $All_Path{$extended_path}{0}=1;
    ...
    foreach my $Scaffold (sort {$a cmp $b} keys %Scaffold_Contig_SEPoint){
	  ...
	  my %Bestn_PathWay=();
	 		 ...
	 		 next if($Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Query}{'I'}<$MinIdentity || $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Query}{'S'}<$MinOverlap);
			   ...
			   for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
			     my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
				     ...
				     $All_Path{$current_path}{$All_Path_Total}=$Scaffold."-Tail-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-MostExtending-Passing";
				     $All_Path_Total++;
			     }
			   }
			   if(@Current_Pacbio_In_Contig==0){
				       ...
				       for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
						...
						if(!exists $All_Path{$Current_Path}){
						    $All_Path{$extended_path}{0}=1;
					            ...
					            $Current_Best++;
						    $All_Path{$extended_path}{$All_Path_Total}=$Scaffold."-Tail-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-MostExtending-Passing";
						    $All_Path_Total++;
						 }
						 else{
						    $All_Path{$extended_path}{0}=1;
		...
		my $Start_Ori="";
                ...
                my $Current_Best=0;
                my %All_Path=();
                foreach my $Query (sort {$Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$b}{'E'} <=> $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$a}{'E'}}keys %$Second_Temp){
                     last if($Current_Best>=$Bestn);
                     next if($Second_Pacbio eq $Query);
                     next if($Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'I'}<$MinIdentity || $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'S'}<$MinOverlap);
                     if(($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==0 && $Start_Ori eq "left") || (($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==1 && $Start_Ori eq "right"))){
                          my $current_path=$Query."-".$Second_Pacbio;
			  my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Query}};
                          my $All_Path_Total=1;
			     ...
			     my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                             ...
                             if((exists $Total_Pacbio_In_Contig{$Query}) && ($Total_Pacbio_In_Contig{$Query}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Query}{'O'} eq "right"))){
                                     ...
                                     $All_Path{$current_path}{$All_Path_Total}=$Scaffold."-Head-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-MostExtending-Passing";
				     $All_Path_Total++;
                             }
			  }
                          if(@Current_Pacbio_In_Contig==0){
					 ...
					 my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                                         ...
                                         if((exists $Total_Pacbio_In_Contig{$key1}) && ($Total_Pacbio_In_Contig{$key1}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$key1}{'O'} eq "right")) ){
                                                if(!exists $All_Path{$Current_Path}){
                                                    ...
                                                    $Current_Best++;
                                                    $All_Path{$extended_path}{$All_Path_Total}=$Scaffold."-Head-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-MostExtending-Passing";
						    $All_Path_Total++;
                                                 }
                                                 else{
                                                    $All_Path{$extended_path}{0}=1;
    ...
    foreach my $Scaffold (sort {$a cmp $b} keys %Scaffold_Contig_SEPoint){
	  ...
	  my %Bestn_PathWay=();
          foreach my $First_Pacbio (keys %$First_Scaffold_Contig){
		   ...
		   my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}};
                   my $All_Path_Total=1;
                   for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
		     my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                     my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                     my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$Second_Real_Pacbios[$pos]}{0};
                     if((exists $Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}) && ($Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$First_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right")) ){
                         $All_Path{$current_path}{0}=1;
		         if(!exists $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){
			         $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
	                 }
				 ...
				 $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}++;
                         ...
                         $All_Path{$current_path}{$All_Path_Total}=$Scaffold."-Tail-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-RandomExtending-Passing";
			 $All_Path_Total++;
                     }
		   }
		   if(@Current_Pacbio_In_Contig==0){
			 ...
			 foreach my $Current_Path (keys %All_Path){
			     ...
			     @Second_Pacbios= sort {$Reads_Pairs{$right_extend}{$Current_Start_Ori}{$b}{'S'} <=> $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$a}{'S'}}keys %$temp;
                             ...
                             for(my $i=0;$i<@Second_Pacbios;$i++){
                                 if(exists $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$Second_Pacbios[$i]}){
                                      ...
                                      next if($right_extend eq $Second_Pacbios[$i]);
			   ...
			   my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}};
                           ...
                           for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
                             ...
                             my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                             my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$Second_Real_Pacbios[$pos]}{0};
                             if( (exists $Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}) && ($Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right")) ){
                                  $All_Path{$extended_path}{0}=1;
		                  if(!exists $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){
			                $Total_Contig_Pairs{$Scaffold}{'Tail'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
	                          }
                                  ...
                                  $All_Path{$extended_path}{$All_Path_Total}=$Scaffold."-Tail-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-RandomExtending-Passing";
				  $All_Path_Total++;
          ...
          foreach my $Second_Pacbio (keys %$Second_Scaffold_Contig){
		     ...
		     my $Start_Ori="";
                     ...
                     my $Current_Best=0;
                     my %All_Path=();
		     my @Second_Pacbios= sort {$Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$b}{'S'} <=> $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$a}{'S'}}keys %$Second_Temp;
		     my @Second_Real_Pacbios=();
		     next if(!@Second_Pacbios);
		     for(my $i=0;$i<@Second_Pacbios;$i++){
			 if(($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==0 && $Start_Ori eq "left") || (($Scaffold_Contig_SEPoint{$Scaffold}{'Head'}{$Second_Pacbio}{0}==1 && $Start_Ori eq "right"))){
                               next if($Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Second_Pacbios[$i]}{'I'}<$MinIdentity || $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Second_Pacbios[$i]}{'S'}<$MinOverlap);
			       next if($Second_Pacbio eq $Second_Pacbios[$i]);
                               push(@Second_Real_Pacbios,$Second_Pacbios[$i]);
			 }
		     }
                     next if(!@Second_Real_Pacbios);
			 ...
			 $pos=int rand($total);
                   ...
                   my $current_path=$Second_Real_Pacbios[$pos]."-".$Second_Pacbio;
		   my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}};
                   my $All_Path_Total=1;
                   for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
		     my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                     my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                     my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$Second_Real_Pacbios[$pos]}{0};
                    if((exists $Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}) && ($Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$Second_Pacbio}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right"))){
                         $All_Path{$current_path}{0}=1;
		         if(!exists $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){
			         $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
	                 }
				 ...
				 $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}++;
                         ...
                         $All_Path{$current_path}{$All_Path_Total}=$Scaffold."-Head-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-RandomExtending-Passing";
		         $All_Path_Total++;
                     }
		   }
		   if(@Current_Pacbio_In_Contig==0){
			 ...
			 foreach my $Current_Path (keys %All_Path){
			     ...
			     @Second_Pacbios= sort {$Reads_Pairs{$right_extend}{$Current_Start_Ori}{$b}{'S'} <=> $Reads_Pairs{$right_extend}{$Current_Start_Ori}{$a}{'S'}}keys %$temp;
				      ...
				      next if($Current_Path=~/$Second_Pacbios[$i]/);
                           ...
                           my $extended_path=$Second_Real_Pacbios[$pos]."-".$Current_Path;
			   my @Current_Pacbio_In_Contig=keys %{$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}};
                           my $All_Path_Total=1;
                           for(my $current_pacbio=0;$current_pacbio<@Current_Pacbio_In_Contig;$current_pacbio++){
			     my $Found_Pacbio_In_Contig=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0};
                             my $Found_Pacbio_In_Ori=$Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{1};
                             my $Found_Pacbio_In_Chain=$Scaffold_Contig_SEPoint{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}{$Second_Real_Pacbios[$pos]}{0};
                             if((exists $Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}) && ($Total_Pacbio_In_Contig{$Second_Real_Pacbios[$pos]}{$Current_Pacbio_In_Contig[$current_pacbio]}{0} ne $Scaffold) && (($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right") || ($Found_Pacbio_In_Chain==0 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Tail" && $Reads_Pairs{$right_extend}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "left") || ($Found_Pacbio_In_Chain==1 && $Found_Pacbio_In_Ori eq "Head" && $Reads_Pairs{$right_extend}{$Start_Ori}{$Second_Real_Pacbios[$pos]}{'O'} eq "right")) ){
                                  $All_Path{$extended_path}{0}=1;
		                  if(!exists $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}){
			                $Total_Contig_Pairs{$Scaffold}{'Head'}{$Found_Pacbio_In_Contig}{$Found_Pacbio_In_Ori}=1;
	                          }
                                  ...
                                  $All_Path{$extended_path}{$All_Path_Total}=$Scaffold."-Head-".$Found_Pacbio_In_Contig."-".$Found_Pacbio_In_Ori."-RandomExtending-Passing";
				  $All_Path_Total++
